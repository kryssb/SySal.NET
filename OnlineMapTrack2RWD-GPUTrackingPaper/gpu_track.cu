...
//			WISE_ALLOC(pTrackGrains, sizeof(IntChain) * C.MaxTracks * _MAX_GRAINS_PER_TRACK_);
			...
			for (isx = C.SlopeCenterX - (C.SlopeAcceptanceX/isstep)*isstep; isx <= C.SlopeCenterX + (C.SlopeAcceptanceX/isstep)*isstep; isx += isstep)
				...
				{				
					...
					}
					{
						...
						//printf("\nDEBUG-TOTALPAIRS %d %d", totalpairs, depth);
			...
			
			{
				...
				int depth = 1;
				while (paircomputersize < hostHashTableBounds.NTBins)
				{
					paircomputersize <<= 1;
					totalpaircomputersize += paircomputersize;
					depth++;
				...
				mergetracks_prepare<<<pcblocks, pcthreads>>>(pTBinFill, hostHashTableBounds.XTBins, hostHashTableBounds.YTBins, pPairComputer);
				_CUDA_THROW_ERR_
				int d;
				int *pin = pPairComputer;
				int *pout;
				...
				if (totalpairs > 0)
				{
					WISE_ALLOC(pPairIndices, sizeof(int) * 2 * totalpairs);
					{
						...
						*/
						...
						mergetracks_mapindex_kernel<<<cblocks, cthreads>>>(pPairIndices, pTBinFill, hostHashTableBounds.XTBins, hostHashTableBounds.YTBins, hostHashTableBounds.TBinCapacity, totalpairs/*, _idz*/);
						//_CUDA_THROW_ERR_
						...
						mergetracks_kernel<<<cblocks, cthreads>>>(pPairIndices, pTBins, C.MergeTrackXYTolerance, C.MergeTrackZTolerance, totalpairs/*, _idz*/);
						//_CUDA_THROW_ERR_
					}
				}
				{
