...
{
	int id = blockIdx.x * blockDim.x + threadIdx.x;
	if (id >= size) return; //poutpairs[id] = 0;
...
__global__ void pair_find1v_kernel_old(int *parrayind, int depth, int *parraycountchains, int *pbinfill, ChainView **ppViewEntryPoints, int *poutchainindices)
{
	int id = blockIdx.x * blockDim.x + threadIdx.x;
...
/* OPTIMIZED
		ifirst = (int)(0.5f * (1.0f + sqrt(1.0f + 8.0f * (float)seek)));
		isecond = seek - ((ifirst * (ifirst - 1)) >> 1);
*/
	}
	/*
	...
	*/
	...
	int i;
	for (i = 0; i < isecond; i++)
		ind = DECODE_CHAIN_INDEX(ppViewEntryPoints, ind)->Reserved;
		//ind = ((IntChain *)(void *)((char *)(void *)ppViewEntryPoints[ind & 1] + (ind & 0xfffffffe)))->Reserved;
