...
#elif (BW_COMPATIBILITY == 0x80) && M_MIL_USE_CE
   typedef       wchar_t               MIL_WCHAR;
#else
   typedef       unsigned short        MIL_WCHAR;
#endif
   #if M_MIL_USE_64BIT || (M_MIL_USE_CE && (BW_COMPATIBILITY == 0x80))
...
typedef MIL_INT MIL_MTXDMA_HANDLE;
typedef long DIG_CONTROL_TYPE;
#define DIG_CONTROL_TYPE_IS_INT64 0
#else
...
///////////////////////////////////////////////////////////////////////////
#if M_MIL_USE_CE && (BW_COMPATIBILITY == 0x80)
...
#elif M_MIL_USE_64BIT /*|| (_MSC_VER >= 1400)*/
...
/***************************/
   ...
   typedef wchar_t*               MIL_TEXT_PTR;

#if M_MIL_USE_CONST_API
   typedef const wchar_t*         MIL_API_CONST_TEXT_PTR;
#else
   typedef wchar_t*               MIL_API_CONST_TEXT_PTR;
#endif
   typedef const wchar_t*         MIL_CONST_TEXT_PTR;
   typedef const wchar_t*         LPCMILSTR;
...
#if (M_MIL_USE_UNICODE && M_MIL_UNICODE_API) || ((BW_COMPATIBILITY == 0x80) && M_MIL_USE_CE)
...
// #define     MOs_vscanf        vwscanf  NOT SUPPORTED ON ALL PLATFORMS
   ...
   #define     MOs_strtok_s(A, B, C)               wcstok(A, B)
   #define     MOs_ANSIstrtok_s(A, B, C)           strtok(A, B)
   ...
   #define     MOs_itoa_s(A,B,C,D)                 _itow(A,B,D)
   ...
   #define     MOs_strtok_s(A, B, C)               strtok(A, B)
   #define     MOs_ANSIstrtok_s(A, B, C)           strtok(A, B)
   ...
   #else
      MIL_INT strnicmp(LPCMILSTR s1, LPCMILSTR s2, MIL_INT n);
      #define     MOs_strnicmp                        strnicmp
   #endif
...
//      #define     MOs_vscanf   vscanf NOT SUPPORTED ON ALL PLATFORMS
      #if M_MIL_USING_SAFE_CRT
            #define     MOs_printf_s                  printf_s
            #define     MOs_itoa_s(val,str,size,rad)  sprintf_s(str, size, "%d", (int)val)
      #else
            #define     MOs_printf_s                  printf
            #define     MOs_itoa_s(val,str,size,rad)  sprintf(str, "%d", (int)val)
      #endif
   #else
...
//      #define     MOs_vscanf      vscanf NOT SUPPORTED ON ALL PLATFORMS
      #if M_MIL_USING_SAFE_CRT
         #define     MOs_printf_s                        printf_s
         #define     MOs_itoa_s                          _itoa_s
      #else
         #define     MOs_printf_s                        printf
         #define     MOs_itoa_s(A,B,C,D)                 _itoa(A,B,D)
      #endif
   #endif
   #if M_MIL_USE_WINDOWS
...
//   #define     MOs_vscanf  vscanf NOT SUPPORTED ON ALL PLATFORMS
   ...
   #define MB_MILINFO                  0x00000006L // specific dialog
   #define MB_MILPROGRESS              0x00000007L // specific dialog
...
/* Safe type is activated by default in C++ and in Debug.
   ...
   #elif (BW_COMPATIBILITY == 0x80)
      #define M_MIL_USE_SAFE_TYPE 0 /* Disable SafeType when compiling for MIL 8.0. */
