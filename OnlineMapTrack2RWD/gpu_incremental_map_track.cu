#define SHOWALLOC_ENABLER if (m_Verbosity >= 2)
...
*/
	...
	bool PrismMapTracker::IsFirstViewEmpty()
	{
		return m_Tracker.IsFirstViewEmpty();
	}

	bool PrismMapTracker::IsLastViewEmpty()
	{
		return m_Tracker.IsLastViewEmpty();
	}
	...
	bool PrismMapTracker::ClusterChainer::SetReferenceZs(SySal::IntClusterFile &cf, bool istop) 
	{
		...
		else if (t.Valid || b.Valid)
		{
			double w = t.Z - b.Z;
			if (w > WeakThickness) WeakThickness = w;
		}
	...
	{
		if (m_Chainer.ThicknessSamples <= 0) 
		{
			if (m_Chainer.WeakThickness > 0.0) return m_Chainer.WeakThickness;
			//throw "No thickness info available.";
			return 0.0;
		}
