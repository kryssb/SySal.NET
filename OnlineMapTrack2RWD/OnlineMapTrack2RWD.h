...
namespace GPU 
{
	...
	private:
		...
		System::String ^DebugDumpTemplate;
	...
	public:
		...
		void SetDebugMark(long long debugmark);
		void SetDebugDumpTemplate(System::String ^debdumptemplate);
		bool inline IsDebugDumpEnabled() { return EnableDebugDump; }
		void SetVerbosity(int v);
		bool inline IsFirstViewEmpty() { return pTk->IsFirstViewEmpty(); }
		bool inline IsLastViewEmpty() { return pTk->IsLastViewEmpty(); }

		MapTracker();
		virtual ~MapTracker();
		!MapTracker();
		void FindTracks(cli::array<System::String ^> ^inputfiles, bool istop, float zsideoffset, SySal::DAQSystem::Scanning::IntercalibrationInfo ^intinfo, unsigned firstview, unsigned lastview);
	};
}
}
