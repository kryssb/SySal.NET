...
#define TRACE_PROLOG3 if (pThis->m_Verbosity >= 3) printf("\n%d DEBUG-TRACE %llx %d ", __LINE__, pThis->GetDebugMarker(), pThis->m_DeviceId),
#define TRACE_PROLOG2 if (pThis->m_Verbosity >= 2) printf("\nDEBUG GPU %d ", pThis->m_DeviceId), 
#define TRACE_PROLOG1 if (pThis->m_Verbosity >= 1) printf("\nGPU %d ", pThis->m_DeviceId), 
...
#define _CUDA_THROW_ERRMSG_(errcode, msg) { cudaDeviceSynchronize(); if (err = cudaGetLastError()) { sprintf(pThis->LastError, "Error \"%s\" at file %s, line %d", cudaGetErrorString(err), __FILE__, __LINE__); printf("\n\n%s %s %d", pThis->LastError, __FILE__, __LINE__); if (err == errcode) printf("\n\n%s", msg); throw pThis->LastError; } }
#define _CUDA_ONLOG2_THROW_ERR_ if (pThis->m_Verbosity >= 2) _CUDA_THROW_ERR_
...
#ifndef SHOWALLOC_ENABLER 
#define SHOWALLOC_ENABLER TRACE_PROLOG2
#endif

#define SHOW_ALLOC(ptr, req) { SHOWALLOC_ENABLER printf("\nGPU Ptr %s %016X Req %lld Mem %lld", # ptr, ptr, (long long)req, _MEM_(ptr)); }
#define HOST_SHOW_ALLOC(ptr) { SHOWALLOC_ENABLER printf("\nHost Ptr %s %016X Mem %lld", # ptr, (long long)ptr, _MEM_(ptr)); }
...
#define WISE_ALLOC_EXCEPT(ptr, m, exc, excmsg) \
{\
	if (_MEM_(ptr) < m) \
	{\
		DEALLOC(ptr); \
		...
		{\
			ptr = 0; \
			sprintf(pThis->LastError, "Error allocating %lld bytes at file %s line %d: %s \n Exception action performed %s.", WISE_MEMSIZE(m), __FILE__, __LINE__, cudaGetErrorString(err), excmsg); \
			printf("\n\n%s", pThis->LastError); \
			TRACE_PROLOG1 exc; \
		}\
		else _MEM_(ptr) = WISE_MEMSIZE(m); \
		SHOW_ALLOC(ptr, m) \
	}\
}
