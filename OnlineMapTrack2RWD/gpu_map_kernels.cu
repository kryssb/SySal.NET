...
__global__ void makefinaldeltas_fromshift_kernel(int *pDeltas, int tol, int *pBestDeltas, int *pBest, int bestdeltasx, short *px, short *py, short *pdx, short *pdy, int img, int totalimg, short *ptotal)
{
	int best = *pBest & 0xffff;
...
__global__ void average_deltas(short *pdx, short *pdy, int totalimg, short *ptotal)
{
	short total = *ptotal;
	if (total > 0)
	{
...
__global__ void makechain_kernel(IntCluster *pC, int totalclusters, short w2, short h2, short *pClusterXs, short *pClusterYs, short *pClusterZs, int xslant, int yslant, IntCluster **pClusterChains, short minclusters, short minvol, float xtomicron, float ytomicron, int stagex, int stagey, IntChain *pChain, int viewtag, int *pvalid, short *pdx, short *pdy)
{
	int id = (blockIdx.y * gridDim.x + blockIdx.x) * blockDim.x + threadIdx.x;
	if (id >= totalclusters) return;	
	...
	int volume = area;
	avgx = (pnc->X + pClusterXs[id] + *pdx) * area;
	avgy = (pnc->Y + pClusterYs[id] + *pdy) * area;		
...
__global__ void compactchains_kernel(IntChain *pcmpct, IntChain *pch, IntPair *pPairs, int totalpairs, ChainView *pChV, int maxchains)
{
	int id = (blockIdx.y * gridDim.x + blockIdx.x) * blockDim.x + threadIdx.x;	
