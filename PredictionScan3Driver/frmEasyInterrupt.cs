...
namespace SySal.DAQSystem.Drivers.PredictionScan3Driver
{
	...
	internal class frmEasyInterrupt : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.Text = "Easy Interrupt for PredictionScan3Driver";
		...
		private void OnBatchManagerSelected(object sender, System.EventArgs e)
		{
			...
			System.Data.DataSet ds = new System.Data.DataSet();
			new SySal.OperaDb.OperaDbDataAdapter("SELECT TB_PROC_OPERATIONS.ID FROM TB_PROC_OPERATIONS INNER JOIN TB_PROGRAMSETTINGS ON (TB_PROC_OPERATIONS.ID_PROGRAMSETTINGS = TB_PROGRAMSETTINGS.ID AND TB_PROGRAMSETTINGS.EXECUTABLE = 'PredictionScan3Driver.exe') WHERE TB_PROC_OPERATIONS.ID IN (" + wherestr + ")", Conn, null).Fill(ds);
			foreach (System.Data.DataRow dr in ds.Tables[0].Rows)
