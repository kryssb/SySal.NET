		...
		SySal::ClusterChainer::EmulsionFocusInfo PrismMapTracker::ClusterChainer::AddClusters(SySal::IntClusterFile &cf)
		{
			...
			for (img = 0; img < cf.Images() - 1; img++)
			{
				...
				}	
				{
					...
					int clusterblocksize = (int)ceil((double)cf.ImageClusterCounts(img) / (ithreads.x * iblocks.x));
				...
				{						
					...
					trymap2_shiftmatch_kernel<<<iblocks, ithreads>>>(pMapClusters, pPairs, totalpairs, pDeltas, cellsize, ncellsx, ncellsy, pPairMatchResult, C.ClusterMapCoarseTolerance, pCells, pCellContents, C.MaxCellContent);
					_CUDA_THROW_ERR_						
				...
				}
				{
					...
					{					
						...
						dim3 ithreads, iblocks;
						pThis->make_threads_blocks(__max(1, totalpairs), ithreads, iblocks);
						iblocks.y = refinedeltas;
						iblocks.z = refinedeltas;
						//printf("\nDEBUG Z3 dev %d  X Y S2 %d %d %d", pThis->m_DeviceId, refinedeltas, refinedeltas, refinedeltas2);
						trymap2_shiftmatch_kernel<<<iblocks, ithreads>>>(pMapClusters, pPairs, totalpairs, pDeltas + deltasX + deltasY, cellsize, ncellsx, ncellsy, pPairMatchResult, C.ClusterMapCoarseTolerance, pCells, pCellContents, C.MaxCellContent);
						_CUDA_THROW_ERR_						
...

			{
				...
				makechain_kernel<<<iblocks, ithreads>>>(
					...
					cf.StageX(0) * (1 << XY_SCALE_SHIFT), cf.StageY(0) * (1 << XY_SCALE_SHIFT), pChains, CurrentView, pClustersInCell);
				_CUDA_THROW_ERR_
				if (CurrentView == 0)
				{
					...
					compactchains_kernel<<<iblocks, ithreads>>>(pCompactChains, pChains, pPairs, itotalpairs, pThisView);
					_CUDA_THROW_ERR_
/*
					{
			...
			if (lastcounts > 0)
			{
				...
				{						
					...
					trymapchain_shiftmatch_kernel<<<iblocks, ithreads>>>(pMapChains, pPairs, totalpairs, pMapCounts, 
						pDeltas, pChMapWnd, C.ChainMapXYCoarseTolerance, deltasZ, C.ChainMapZCoarseTolerance);
					_CUDA_THROW_ERR_						
					pThis->make_threads_blocks(deltasXYZ, ithreads, iblocks);
				...
				{						
					dim3 ithreads, iblocks;
					pThis->make_threads_blocks(__max(1, totalpairs), ithreads, iblocks);
					iblocks.y = refinedeltasXY;
					iblocks.z = refinedeltasXY;
					trymapchain_shiftmatch_kernel<<<iblocks, ithreads>>>(pMapChains, pPairs, totalpairs, pMapCounts, 
						pDeltas + deltasX + deltasY + 3 * deltasZ, pChMapWnd, C.ChainMapXYFineAcceptance, refinedeltasZ, C.ChainMapZFineAcceptance);
					_CUDA_THROW_ERR_						
					pThis->make_threads_blocks(refinedeltasXYZ, ithreads, iblocks);
				...
				}
				{
					...
					pThis->make_threads_blocks(__max(1, totalpairs), ithreads, iblocks);
					compactchains_kernel<<<iblocks, ithreads>>>(pCompactChains, pChains, pPairs, totalpairs, pThisView);
					_CUDA_THROW_ERR_
/*
					{
...
*/
...

				{
					dim3 ithreads, iblocks;
					pThis->make_threads_blocks(__max(1, lastcounts), ithreads, iblocks);					
					clearhashchain2_kernel<<<iblocks, ithreads>>>(pLastView, pChMapWnd, 1/*C.ChainMapSampleDivider*/);
					_CUDA_THROW_ERR_
					{
						int xyz[3];
