...
{	
	namespace Control
	{
		...
		/// GDI-based display.
		/// </summary>
		public class GDIDisplay : System.Windows.Forms.UserControl
		{
			...
			internal MouseMotion m_MouseMotion = MouseMotion.Rotate;
			/// <summary>
			/// Gets/sets the mouse motion mode.
			/// </summary>
			public MouseMotion MouseMode
			{
				get { return m_MouseMotion; }
				set 
				{
					m_MouseMotion = value;
				}
			...
			internal bool m_NextClickSetsCenter = false;
			/// <summary>
			...
			/// </summary>
			public bool NextClickSetsCenter
			{
				get { return m_NextClickSetsCenter; }
				set { m_NextClickSetsCenter = value; }
			}
            ...
            internal void SaveMetafile(string filename)
            {
                ...
                IntPtr hdc = g.GetHdc();
                System.Drawing.Imaging.Metafile mf = new System.Drawing.Imaging.Metafile(filename, hdc, new Rectangle(0, 0, bmp.Width, bmp.Height), System.Drawing.Imaging.MetafileFrameUnit.Pixel);
                g.ReleaseHdc();
                g.Dispose();
			...
			public void LoadScene(string filename)
			{
				...
				System.Xml.Serialization.XmlSerializer xmls = new System.Xml.Serialization.XmlSerializer(typeof(GDI3D.Scene));
				GDI3D.Scene scene = null; 
				try
				{
					rd = new System.IO.StreamReader(filename);
					scene = (GDI3D.Scene)xmls.Deserialize(rd);
					rd.Close();
					SetScene(scene);
				}
				catch (Exception x)
			...
			/// </summary>
			/// <param name="filename">Name of the file from which the image must be loaded.</param>
			public void LoadMergeScene(string filename)
			{
				System.IO.StreamReader rd = null;
				System.Xml.Serialization.XmlSerializer xmls = new System.Xml.Serialization.XmlSerializer(typeof(GDI3D.Scene));
				GDI3D.Scene scene = null; 
				try
				{
					...
					rd.Close();
					int i;
					bool saveautorender = m_AutoRender;
					m_AutoRender = false;
					if (scene.Points != null)
						for (i = 0; i < scene.Points.Length; i++)
							Add(new GDI3D.Control.Point(scene.Points[i].X, scene.Points[i].Y, scene.Points[i].Z, (scene.Points[i].Owner < 0) ? null : scene.OwnerSignatures[scene.Points[i].Owner], scene.Points[i].R, scene.Points[i].G, scene.Points[i].B));
					if (scene.Lines != null)
						for (i = 0; i < scene.Lines.Length; i++)
							Add(new GDI3D.Control.Line(scene.Lines[i].XF, scene.Lines[i].YF, scene.Lines[i].ZF, scene.Lines[i].XS, scene.Lines[i].YS, scene.Lines[i].ZS, (scene.Lines[i].Owner < 0) ? null : scene.OwnerSignatures[scene.Lines[i].Owner], scene.Lines[i].R, scene.Lines[i].G, scene.Lines[i].B));
					m_AutoRender = saveautorender;
					m_Changed = true;
					if (m_AutoRender) Render();
				}
				catch (Exception x)
			...
			public GDIDisplay()
			{
				...
				BufferBitmap = new Bitmap(this.Width, this.Height, System.Drawing.Imaging.PixelFormat.Format32bppArgb);
				BufferG = System.Drawing.Graphics.FromImage(BufferBitmap);
                m_MovieTimer.Tick += new EventHandler(m_MovieTimer_Tick);
			}
			...
			{
                System.Drawing.Drawing2D.SmoothingMode osm = (g == null) ? System.Drawing.Drawing2D.SmoothingMode.None : g.SmoothingMode;                
                try
                {
                    ...
                    foreach (Line l in m_Lines)
                    {
                        ...
                        if (l.EnableLabel && l.Label != null && l.Label.Length > 0) g.DrawString(l.Label, m_LabelFont, new SolidBrush(Color.FromArgb(m_Alpha, l.R, l.G, l.B)), (l.TXS + l.TXF) / 2 + m_LabelOffsetX, (l.TYS + l.TYF) / 2 + m_LabelOffsetY);
                    }
                }
                finally
                {
                    if (g != null) g.SmoothingMode = osm;
                }
			}
            ...
            /// </summary>
            protected Movie m_AutoMovie = null;
            /// <summary>
            /// The maximum size of a movie.
            /// </summary>
            protected int m_SizeLimitKB = 10240;

            /// <summary>
            /// Movie timer for this control.
            /// </summary>
            protected Timer m_MovieTimer = new Timer();
            ...
            public void StartMovie(int millisec, int sizelimitKB)
            {
                m_MovieTimer.Stop();
                m_AutoMovie = new Movie((int)Math.Ceiling(millisec * 0.01));
                m_MovieTimer.Interval = millisec;                                
                m_MovieTimer.Start();
            }

            void m_MovieTimer_Tick(object sender, EventArgs e)
            {
                if (m_AutoMovie != null && m_AutoMovie.Size < m_SizeLimitKB * 1024)
                {
                    Render();
                    m_AutoMovie.AddFrame(BufferBitmap);
                }
            }
            ...
            public Movie StopMovie(string filename)
            {
                m_MovieTimer.Stop();
                if (filename != null)
                    m_AutoMovie.Save(filename);
                return m_AutoMovie;
            }
			...
			{
				int i;
				double zoom = Math.Min(m_Zoom * ClientRectangle.Width, m_Zoom * ClientRectangle.Height);
				int xc = ClientRectangle.Width / 2 + ClientRectangle.Left;
				int yc = ClientRectangle.Height / 2 + ClientRectangle.Top;
				for (i = 0; i < m_Points.Count; i++)
				{
					Point p = (Point)m_Points[i];
			...
			internal int LastMouseX;
			internal int LastMouseY;

			internal double m_MouseMultiplier = 0.01;
			/// <summary>
			/// Gets/sets mouse rotation/panning multiplier.
			/// </summary>
			public double MouseMultiplier
			{
				get { return m_MouseMultiplier; }
				set { m_MouseMultiplier = value; }
			}
			...
			private void OnMouseMove(object sender, System.Windows.Forms.MouseEventArgs e)
			{
				...
				double DX = (LastMouseX - e.X) * m_MouseMultiplier;
				double DY = (LastMouseY - e.Y) * m_MouseMultiplier;
				LastMouseX = e.X;
				LastMouseY = e.Y;
				switch (m_MouseMotion)
				{
					case MouseMotion.Pan:	this.SetCameraSpotting(
												m_CameraSpottingX + (DX * m_CameraBinormalX + DY * m_CameraNormalX) * Math.Sqrt(m_Distance), 
												m_CameraSpottingY + (DX * m_CameraBinormalY + DY * m_CameraNormalY) * Math.Sqrt(m_Distance),
												m_CameraSpottingZ + (DX * m_CameraBinormalZ + DY * m_CameraNormalZ) * Math.Sqrt(m_Distance)
												);
						break;

					case MouseMotion.Rotate: 
					{
						double AX = -DX * m_CameraNormalX + DY * m_CameraBinormalX;
						double AY = -DX * m_CameraNormalY + DY * m_CameraBinormalY;
						double AZ = -DX * m_CameraNormalZ + DY * m_CameraBinormalZ;				
						RotateVector(ref m_CameraDirectionX, ref m_CameraDirectionY, ref m_CameraDirectionZ, AX, AY, AZ);
						RotateVector(ref m_CameraNormalX, ref m_CameraNormalY, ref m_CameraNormalZ, AX, AY, AZ);
						RotateVector(ref m_CameraBinormalX, ref m_CameraBinormalY, ref m_CameraBinormalZ, AX, AY, AZ);
						SetCameraSpotting(m_CameraSpottingX, m_CameraSpottingY, m_CameraSpottingZ);
					}
						break;
				}
			}
			...
			internal static void RotateVector(ref double VX, ref double VY, ref double VZ, double AX, double AY, double AZ)
			{
				...
				double aY = AY * AW;
				double aZ = AZ * AW;
				double vN = VX * aX + VY * aY + VZ * aZ;
				double vX = VX - vN * aX;
				double vY = VY - vN * aY;
				double vZ = VZ - vN * aZ;
				double wX = vY * aZ - vZ * aY;
				double wY = vZ * aX - vX * aZ;
				double wZ = vX * aY - vY * aX;
			...
			internal SelectObject m_ClickSelect = null;

			/// <summary>
			/// Gets/sets the callback for Click selection.
			/// </summary>
			public SelectObject ClickSelect
			{
				get { return m_ClickSelect; }
				set { m_ClickSelect = value; }
			}

			internal SelectObject m_DoubleClickSelect = null;

			/// <summary>
			/// Gets/sets the callback for Double-Click selection.
			/// </summary>
			public SelectObject DoubleClickSelect
			{
				get { return m_DoubleClickSelect; }
				set { m_DoubleClickSelect = value; }
			}

			private void OnDoubleClick(object sender, System.EventArgs e)
			{
				if (m_DoubleClickSelect != null)
				{
					object o = null;
					...
					foreach (Point p in m_Points)
					{
						...
						d = p.Distance2(LastMouseX, LastMouseY);
						if (o == null || d < bestd)
						{
							bestd = d;
							o = p.Owner;
					...
					foreach (Line l in m_Lines)
					{
						...
						d = l.Distance2(LastMouseX, LastMouseY);
						if (o == null || d < bestd)
						{
							bestd = d;
							o = l.Owner;
					...
					if (o != null) m_DoubleClickSelect(o);
				}
			}

			private void OnClick(object sender, System.EventArgs e)
			{
				if (m_ClickSelect != null || m_NextClickSetsCenter == true)
				{
					object o = null;
					double d = 0.0, bestd = 0.0;
					...
					if (m_NextClickSetsCenter)
					{
						m_NextClickSetsCenter = false;
						SetCameraSpotting(x, y, z);
					}
					else if (o != null) m_ClickSelect(o);
				}		
			}
			/// <summary>
			/// Gets the scene.
		...
		public delegate void SelectObject(object obj);
	}
}
